...
<div class="container mt-4 mb-5">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h3>✏️ Cập Nhật Tour: <?= htmlspecialchars($tour['ten_tour']) ?></h3>
            ...
            <form action="index.php?action=admin-tour-update" method="POST" enctype="multipart/form-data">
                ...
                <div class="tab-content mt-4" id="tourTabContent">
                    ...
                    <div class="tab-pane fade show active" id="general" role="tabpanel">
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="loai_tour_id" class="form-select">
                                    <?php foreach ($categories as $cate): ?>
                                        <option value="<?= $cate['id'] ?>" <?= $cate['id'] == $tour['loai_tour_id'] ? 'selected' : '' ?>><?= $cate['ten_loai'] ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="col-md-4">
                            ...
                            <div class="col-12">
                                ...
                                <textarea name="gioi_thieu" class="form-control" rows="3"><?= $tour['gioi_thieu'] ?></textarea>
                            </div>
                            <div class="col-12">
                                <label class="form-label fw-bold">Lịch trình tóm tắt</label>
                                <input type="text" name="lich_trinh_tom_tat" class="form-control" value="<?= $tour['lich_trinh'] ?>">
                            </div>
                            <div class="col-12">
                                <label class="form-label fw-bold">Ảnh đại diện</label>
                                <input type="file" name="anh_tour" class="form-control">
                                <?php if($tour['anh_tour']): ?><img src="assets/uploads/<?= $tour['anh_tour'] ?>" width="100" class="mt-2 img-thumbnail"><?php endif; ?>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="tab-pane fade" id="itinerary" role="tabpanel">
                        <div id="itinerary-wrapper">
                            ...
                            <div class="card mb-3 border bg-light">
                                ...
                                <div class="card-body">
                                    <div class="mb-3"><label class="form-label">Tiêu đề</label><input type="text" name="itinerary_title[]" class="form-control bg-white" value="<?= $day['tieu_de'] ?>" required></div>
                                    <div class="mb-0"><textarea name="itinerary_content[]" class="form-control bg-white" rows="3"><?= $day['noi_dung'] ?></textarea></div>
                                </div>
                            </div>
                            <?php endforeach; ?>
                        </div>
                    ...
                    <div class="tab-pane fade" id="suppliers" role="tabpanel">
                        <div class="alert alert-info">Tick chọn các nhà cung cấp sẽ phục vụ cho tour này.</div>
                        <div class="table-responsive">
                            <table class="table table-bordered">
                                <thead class="table-light">
                                    <tr><th width="50">Chọn</th><th>Tên Nhà Cung Cấp</th><th>Dịch vụ</th><th>Ghi chú</th></tr>
                                </thead>
                                <tbody>
                                    <?php if(!empty($allSuppliers)): ?>
                                        ...
                                        <tr>
                                            ...
                                            <td><input type="text" name="suppliers_note[<?= $s['id'] ?>]" class="form-control form-control-sm" value="<?= $noteVal ?>"></td>
                                        </tr>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                                </tbody>
                    ...
                    <div class="tab-pane fade" id="gallery" role="tabpanel">
                        <div class="mb-3"><label class="form-label fw-bold">Thêm ảnh mới</label><input type="file" name="gallery[]" class="form-control" multiple></div>
                        <div class="row mt-3">
                            <p class="fw-bold">Ảnh hiện tại:</p>
                            <?php foreach ($gallery as $img): ?><div class="col-md-2 col-4 mb-3"><img src="assets/uploads/<?= $img['image_url'] ?>" class="img-fluid img-thumbnail"></div><?php endforeach; ?>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    function addDay() {
        dayCount++;
        const html = `<div class="card mb-3 border bg-light"><div class="card-header py-2"><strong>Ngày ${dayCount} (Mới)</strong></div><div class="card-body"><div class="mb-3"><label class="form-label">Tiêu đề</label><input type="text" name="itinerary_title[]" class="form-control bg-white" required></div><div class="mb-0"><textarea name="itinerary_content[]" class="form-control bg-white" rows="3"></textarea></div></div></div>`;
