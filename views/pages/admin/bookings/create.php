...
<body class="bg-light">
                                        ...
                                        <?php if (isset($schedules) && is_array($schedules)): ?>
                                            <?php foreach ($schedules as $item): ?>
                                                <option value="<?= $item['id'] ?>"
                                                    ...
                                                    data-slots="<?= $item['so_cho_toi_da'] - $item['so_cho_da_dat'] ?>"> <?= $item['ten_tour'] ?> |
    ...
    // Hàm lấy số chỗ còn trống của Tour đang chọn
    function getRemainingSlots() {
        ...
        // Nếu chưa chọn tour (value rỗng) thì trả về 0 hoặc số lớn tùy logic (ở đây để 0 để bắt buộc chọn)
        if (!selectTour.value) return 0;

        return parseInt(selectedOption.getAttribute('data-slots')) || 0;
    }

    function addMemberRow() {
        // 1. Kiểm tra xem đã chọn Tour chưa
        const selectTour = document.getElementById('select_tour');
        if (!selectTour.value) {
            alert("Vui lòng chọn Tour và Lịch khởi hành trước khi thêm khách!");
            selectTour.focus();
            return;
        }

        // 2. Lấy giới hạn chỗ và số khách hiện tại
        const maxSlots = getRemainingSlots();
        const currentMembers = document.querySelectorAll('.member-row').length;

        // 3. Kiểm tra: Nếu số khách hiện tại >= số chỗ trống -> Chặn
        if (currentMembers >= maxSlots) {
            alert(`Tour này chỉ còn trống ${maxSlots} chỗ! Không thể thêm khách nữa.`);
            return; // Dừng lại, không thêm dòng mới
        }

        // --- Logic thêm dòng cũ giữ nguyên ---
    ...
    // Hàm này chạy khi chọn/đổi Tour
    function calculatePrice() {
        const selectTour = document.getElementById('select_tour');
        const selectedOption = selectTour.options[selectTour.selectedIndex];

        const maxSlots = parseInt(selectedOption.getAttribute('data-slots')) || 0;
        const currentMembers = document.querySelectorAll('.member-row').length;

        if (selectTour.value && currentMembers > maxSlots) {
            alert(`CẢNH BÁO: Tour bạn vừa chọn chỉ còn ${maxSlots} chỗ, nhưng danh sách đang có ${currentMembers} khách.\nVui lòng xóa bớt khách để tiếp tục!`);
        }
